...
- `kagi_connection()`
  Build a reusable authenticated connection (Kagi `v1` API; `Bearer` auth).

## Query Construction

- `kagi_query_search()`
  Build Search endpoint requests (workflows: `search`, `images`, `videos`,
  `news`, `podcasts`).
- `kagi_query_extract()`
  Build Extract endpoint requests (URL → markdown).
...
- `as_corpus_parquet()`
  Project a search parquet dataset to a `(id, title, abstract)` corpus.

## Utility

- `kagi_open_search_query()`
  Open a `kagi_query_search` object (or list of them) in the browser for
  inspection.
